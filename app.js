...
function renderRoute() {
  ...
  else if (route === "horizon") renderLongHorizon(root);
...
window.addEventListener("deeproject:event", event => {
  ...
  if (event.detail.type === "task.scenario_completed" || event.detail.type === "evaluation.decision_recorded") syncScenarioReview();
});
window.addEventListener("deeproject:saved", () => { document.querySelector("#saveStatus").textContent = "Saved locally"; });
window.addEventListener("hashchange", () => {
  const route = location.hash.slice(1);
  if (["scenario", "framework", "horizon", "review"].includes(route)) {
...
if (["scenario", "framework", "horizon", "review"].includes(initialRoute)) mutate(s => { s.route = initialRoute; }, null);
updateChrome();
renderRoute();
